...
async fn diagnose_testnet_endpoints() {
    ...
    let endpoints = vec![
        ...
        ("wss://pnn.kaspa.stream/borsh/testnet-10", "PNN borsh/testnet-10"),
        ("wss://pnn.kaspa.stream/testnet-10/borsh", "PNN testnet-10/borsh"),
        ("wss://pnn.kaspa.stream/wrpc/borsh/testnet-10", "PNN wrpc/borsh/testnet-10"),
        ("wss://pnn.kaspa.stream/wrpc/testnet-10/borsh", "PNN wrpc/testnet-10/borsh"),

        // Aspectron endpoints
        ("wss://kaspa.aspectron.com/borsh/testnet-10", "Aspectron borsh/testnet-10"),
        ("wss://kaspa.aspectron.com/testnet-10/borsh", "Aspectron testnet-10/borsh"),
        ("wss://kaspa.aspectron.com/wrpc/borsh/testnet-10", "Aspectron wrpc/borsh/testnet-10"),

        // Short network names
        ("wss://pnn.kaspa.stream/borsh/tn10", "PNN borsh/tn10"),
        ("wss://kaspa.aspectron.com/borsh/tn10", "Aspectron borsh/tn10"),

        // JSON encoding (as fallback)
        ("wss://pnn.kaspa.stream/json/testnet-10", "PNN json/testnet-10"),
        ("wss://kaspa.aspectron.com/json/testnet-10", "Aspectron json/testnet-10"),

        // Mainnet (to verify connectivity works at all)
        ("wss://pnn.kaspa.stream/borsh/mainnet", "PNN borsh/mainnet"),
        ("wss://kaspa.aspectron.com/borsh/mainnet", "Aspectron borsh/mainnet"),
    ];
    ...
    println!();
    println!("═══════════════════════════════════════════════════════════");
    println!("SUMMARY: {} working endpoint(s) found", working_endpoints.len());
    println!("═══════════════════════════════════════════════════════════");
            ...
            if response.status().is_success() {
                match response.json::<Vec<serde_json::Value>>().await {
                    Ok(nodes) => {
                        ...
                        for node in &nodes {
                            let network = node.get("network").and_then(|v| v.as_str()).unwrap_or("");
                        ...
                        for node in &nodes {
                            let network = node.get("network").and_then(|v| v.as_str()).unwrap_or("");
                            let status = node.get("status").and_then(|v| v.as_str()).unwrap_or("");
                            let encoding = node.get("encoding").and_then(|v| v.as_str()).unwrap_or("");

                            if network == "testnet-10" && (status == "online" || status == "delegator") && encoding == "borsh" {
                                ...
                                let clients = node.get("clients").and_then(|v| v.as_u64()).unwrap_or(0);
                                println!("  - SID: {}  UID: {}  peers: {}  clients: {}  status: {}",
                                    &sid[..sid.len().min(16)], &uid[..uid.len().min(16)], peers, clients, status);
                            }
                        }
                    }
                    Err(e) => {
